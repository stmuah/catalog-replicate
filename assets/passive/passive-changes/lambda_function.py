...
isActive = (os.environ['ACTIVE']=='Active')
...
botoSession = Session(region_name=target_region)
...
eb = botoSession.client('events')#,config=my_config)
sqs = botoSession.client('sqs')#,config=my_config)
glue = botoSession.client('glue')#,config=my_config)


def lambda_handler(event, context):
    
    target_account_id = context.invoked_function_arn.split(":")[4]
    ...
    try:
        ...
        detail_type=catalog_change['detail-type']
        ...
        print('West Event Destined for East:\n',catalog_change)
        entries = []
        for msg in metaData:
        ...
        print(eb.put_events(Entries=entries))
